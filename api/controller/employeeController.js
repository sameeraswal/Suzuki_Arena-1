...
exports.registerEmployee = async (req, res) => {
    ...
    try {
        ...
        const checkemployeeExists = await Employee.find({ mspin: mspin, name: name, dealership: dealership });
        if (!checkemployeeExists.length) {
            ...
            const employeeDetails = await Employee.find({ mspin: mspin, name: name, dealership: dealership, registrationNumber: registrationNumber });
            console.log(employeeDetails);
            if (employeeDetails.length) {
                return res.json({
                    status: status,
                    message: "employee is already registered with this mspin and registration number"
                })
                // status = true;
                // const name = employeeDetails.name;
                // const dealership = employeeDetails.dealership;
                ...
                const regNumberSaved = await Employee.updateOne({ mspin: mspin, name: name, dealership: dealership }, { "$set": { registrationNumber: registrationNumber } });
